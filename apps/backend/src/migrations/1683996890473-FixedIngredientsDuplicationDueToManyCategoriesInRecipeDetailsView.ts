...
export class FixedIngredientsDuplicationDueToManyCategoriesInRecipeDetailsView1683996890473
  implements MigrationInterface
{
  name =
    'FixedIngredientsDuplicationDueToManyCategoriesInRecipeDetailsView1683996890473';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      ...
      [
        ...
        'SELECT "recipe"."id" AS id, "recipe"."name" AS name, "recipe"."description" AS description, "recipe"."portions" AS portions, "recipe"."instructions" AS instructions, "recipe"."state" AS state, "recipe"."difficulty" AS difficulty, "recipe"."preparation_time" AS "preparationTime", "recipe"."author_id" AS "authorId", array_remove(array_agg(DISTINCT "recipeCategories"."category_id"), NULL) AS "categoryIds", array_remove(\n                  array_agg(\n                    DISTINCT\n                    CASE\n                    WHEN \n                      "ingredient"."id" IS NOT NULL AND\n                      "ingredient"."name" IS NOT NULL AND \n                      "recipeIngredients".quantity IS NOT NULL AND\n                      "recipeIngredients".unit IS NOT NULL\n                    THEN jsonb_build_object(\n                      \'id\', "ingredient"."id",\n                      \'name\', "ingredient"."name",\n                      \'quantity\',\n                      "recipeIngredients".quantity,\n                      \'unit\', "recipeIngredients".unit\n                    )\n                    END\n                  ), NULL\n                 ) AS "ingredients" FROM "recipes" "recipe" LEFT JOIN "recipe_ingredients" "recipeIngredients" ON "recipe"."id" = "recipeIngredients"."recipe_id"  LEFT JOIN "ingredients" "ingredient" ON "recipeIngredients"."ingredient_id" = "ingredient"."id"  LEFT JOIN "recipe_categories" "recipeCategories" ON "recipeCategories"."recipe_id" = "recipe"."id" GROUP BY "recipe"."id"',
      ],
    );
  }
