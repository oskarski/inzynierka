...
export class AddCoverImageColumnToRecipeListItemView1685036789717
  implements MigrationInterface
{
  name = 'AddCoverImageColumnToRecipeListItemView1685036789717';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE VIEW "recipe_list_item_view_entity" AS SELECT "recipes"."id" AS id, "recipes"."name" AS name, "recipes"."description" AS description, "recipes"."portions" AS portions, "recipes"."state" AS state, "recipes"."difficulty" AS difficulty, "recipes"."review" AS review, "recipes"."preparation_time" AS "preparationTime", "recipes"."author_id" AS "authorId", "recipes"."cover_image" AS "coverImage", array_remove(array_agg("category"."category_id"), NULL) AS "categoryIds" FROM "recipes" "recipes" LEFT JOIN "recipe_categories" "category" ON category."recipe_id" = recipes."id" GROUP BY "recipes"."id" ORDER BY "recipes"."id" ASC`,
    );
    await queryRunner.query(
